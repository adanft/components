  ...
  return (
    <header className="relative flex items-center h-24 p-2">
      <a href={href} className="flex items-center gap-2 overflow-hidden">
        ...
        <span className="font-semibold text-2xl whitespace-nowrap ui-text-brand">{title}</span>
      </a>
      <button
        className={`ui-text-brand leading-none transition-all duration-300 absolute top-2/4 -translate-y-1/2 cursor-pointer border-2 ui-border-brand p-2 rounded-md ${state ? 'left-68.5' : 'left-18.5'}`}
